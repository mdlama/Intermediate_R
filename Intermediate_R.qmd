...
::: {.callout-tip title="Exercise 1: Comparison of logicals"}
*Write R code to see if* `TRUE` *equals* `FALSE`*.*

```{webr}
#| exercise: truthiness-ex-1
#| autorun: false
```

::: { .solution exercise="truthiness-ex-1"}
...
::: {.callout-tip title="Exercise 2: Comparison of numerics"}
*Check if* `-6 * 14` *is not equal to* `17 - 101`*.*

```{webr}
#| exercise: truthiness-ex-2
#| autorun: false
```

::: { .solution exercise="truthiness-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 3: Comparison of character strings"}
*Compare character strings by asking R whether the strings* `"useR"` *and* `"user"` *are equal.*

```{webr}
#| exercise: truthiness-ex-3
#| autorun: false
```

::: { .solution exercise="truthiness-ex-3"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 4: Compare a logical with a numeric"}
*Find out what happens if you compare logicals to numerics: are* `TRUE` *and* `1` *equal?*

```{webr}
#| exercise: truthiness-ex-4
#| autorun: false
```

::: { .solution exercise="truthiness-ex-4"}
::: { .callout-note collapse="false"}
...
#| exercise: truthiness-ex-4
#| check: true
gradethis::grade_this_code("Since `TRUE` coerces to `1` under the hood, `TRUE == 1` evaluates to `TRUE`. Make sure not to mix up `==` (comparison) and `=` (assignment), `==` is needed to check the equality of R objects.")
```
...
::: {.callout-tip title="Exercise 1: Comparison of numerics"}
*Write R code to check whether* `-6 * 5 + 2` *is greater than or equal to* `-10 + 1`*.*

```{webr}
#| exercise: grtorequ-ex-1
#| autorun: false
```

::: { .solution exercise="grtorequ-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: Comparison of character strings"}
*Write R code to check whether* `"raining"` *is less than or equal to* `"raining dogs"`*.*

```{webr}
#| exercise: grtorequ-ex-2
#| autorun: false
```

::: { .solution exercise="grtorequ-ex-2"}
::: { .callout-note collapse="false"}
...
#| exercise: grtorequ-ex-3
#| check: TRUE
gradethis::grade_this_code("Make sure to have a look at the console output to see if R returns the results you expected.")
```
...
::: {.callout-tip title="Exercise 1: Popular days"}
...
::: { .solution exercise="compare-vecs-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: Quiet days"}
...
::: { .solution exercise="compare-vecs-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 3: LinkedIn more popular than Facebook"}
...
#| exercise: compare-vecs-ex-3
#| check: TRUE
gradethis::grade_this_code("Have a look at the console output. Your LinkedIn profile was pretty popular on the sixth day, but less so on the fourth and fifth day.")
```
...
::: {.callout-tip title="Exercise 1: When does views equal 13?"}
...
::: { .solution exercise="compare-mats-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: When is views less than or equal to 14?"}
...
::: { .solution exercise="compare-mats-ex-2"}
::: { .callout-note collapse="false"}
...
#| exercise: compare-mats-ex-2
#| check: TRUE
gradethis::grade_this_code("This exercise concludes the part on comparators. Now that you know how to query the relation between R objects, the next step will be to use the results to alter the behavior of your programs. Find out all about that in the next video!")
```
...
::: {.callout-tip title="Exercise 1: Is last under 5 or above 10?"}
*Write R code to determine if* `last` *is under 5 or above 10.*

```{webr}
#| exercise: and-or-ex-1
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
last <- tail(linkedin, 1)
```
...
::: {.callout-tip title="Exercise 2: Is last between 15 (exclusive) and 20 (inclusive)?"}
*Write R code to determine if* `last` *is between 15 and 20 (excluding 15 but including 20).*

```{webr}
#| exercise: and-or-ex-2
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
last <- tail(linkedin, 1)
```
...
#| exercise: and-or-ex-2
#| check: TRUE
gradethis::grade_this_code("Have one last look at the console before proceeding; do the results of the different expressions make sense?")
```
...
::: {.callout-tip title="Exercise 1: `linkedin` exceeds 10 but `facebook` below 10"}
...
::: { .solution exercise="and-or-vec-mat-ex-1"}
::: { .callout-note collapse="false"}
...
#| exercise: and-or-vec-mat-ex-1
#| check: TRUE
gradethis::grade_this_code("You'll have noticed how easy it is to use logical operators to vectors and matrices. What do these results tell us? The third day of the recordings was the only day where your LinkedIn profile was visited more than 10 times, while your Facebook profile wasn't. Can you draw similar conclusions for the results of the next two exercises?")
```
...
::: {.callout-tip title="Exercise 2: When were one or both visited at least 12 times?"}
*When were one or both of your LinkedIn and Facebook profiles visited at least 12 times?*

```{webr}
#| exercise: and-or-vec-mat-ex-2
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
facebook <- c(17, 7, 5, 16, 8, 13, 14)
views <- matrix(c(linkedin, facebook), nrow = 2, byrow = TRUE)
...
::: { .solution exercise="and-or-vec-mat-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 3: When is views between 11 (exclusive) and 14 (inclusive)?"}
*When is the* `views` *matrix equal to a number between 11 and 14, excluding 11 and including 14?*

```{webr}
#| exercise: and-or-vec-mat-ex-3
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
facebook <- c(17, 7, 5, 16, 8, 13, 14)
views <- matrix(c(linkedin, facebook), nrow = 2, byrow = TRUE)
...
::: { .solution exercise="and-or-vec-mat-ex-3"}
::: { .callout-note collapse="false"}
...
#| echo: FALSE
library(checkdown)
check_question(c("FALSE"), options = c("TRUE", "FALSE", "Running this piece of code would throw an error."), type = "radio", right = "Correct", wrong = "Incorrect")
```
...
::: {.callout-tip title="Exercise 1: Pulling out a column"}
...
::: {.callout-tip title="Exercise 2: Create boolean vector of extremes"}
...
::: {.callout-tip title="Exercise 3: Count the number of TRUEs in extremes"}
...
::: {.callout-important title="Challenge: Solve it with a one-liner"}
...
#| exercise: blend-ex-4
#| check: TRUE
gradethis::grade_this_code("This one liner illustrates that you may or may not need to store the second column as an intermediate variable if you only need to use it once. Head over to the next section and learn how relational and logical operators can be used to alter the flow of your R scripts.")
```
...
::: {.callout-tip title="Exercise 1: Write the if statement for `num_views`"}
...
#| exercise: if-ex-2
#| check: TRUE
gradethis::grade_this_code("Try to see what happens if you change the `medium` and `num_views` variables and run your code again. Let's further customize these if statements in the next exercise.")
```
...
::: {.callout-tip title="Exercise 1: Control structure for `medium`"}
...
::: {.callout-tip title="Exercise 2: Control structure for num_views"}
...
#| exercise: else-ex-2
#| check: TRUE
gradethis::grade_this_code("You also had Facebook information available, remember? Time to add some more statements to our control structures using `else if`!")
```
...
::: { .solution exercise="else-if-ex-1"}
::: { .callout-note collapse="false"}
...
::: { .solution exercise="else-if-ex-2"}
::: { .callout-note collapse="false"}
...
#| exercise: else-if-ex-2
#| check: TRUE
gradethis::grade_this_code("Because R abandons the control flow as soon as it finds a condition that is met, you can simplify the condition for the `else if` part in the second construct to `num_views > 10`.")
```
...
library(checkdown)
check_question(
  c('If number is set to 6, "small" gets printed to the console.',
    'If number is set to 4, "extra small" gets printed out to the console.'), 
  options = c('If number is set to 6, "small" gets printed to the console.', 
...
::: {.callout-tip title="Exercise 1: Control-flow construct"}
...
::: { .solution exercise="control-ex-1"}
::: { .callout-note collapse="false"}
...
#| exercise: control-ex-1
#| check: TRUE
gradethis::grade_this_code("Feel free to play around some more with your solution by changing the values of `li` and `fb`.")
```
...
::: {.callout-tip title="Exercise: Write a while loop"}
...
#| exercise: loop-ex
#| check: TRUE
gradethis::grade_this_code("Proceed to the next exercise.")
```
...
::: {.callout-tip title="Exercise: Throw in more conditionals"}
...
#| exercise: more-ex
#| check: TRUE
gradethis::grade_this_code("To further improve our driver assistant model, head over to the next exercise!")
```
...
::: {.callout-tip title="Exercise: Stop the while loop with break"}
...
#| exercise: break-ex
#| check: TRUE
gradethis::grade_this_code("Now that you've correctly solved this exercise, feel free to play around with different values of `speed` to see how the `while` loop handles the different cases.")
```
...
::: {.callout-tip title="Exercise: Build a while loop from scratch"}
...
::: { .solution exercise="while-scratch-ex"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise: Loop over a vector"}
...
::: { .solution exercise="vector-loop-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: vector-loop-ex
#| check: TRUE
gradethis::grade_this_code("Please go to the next exercise.")
```
...
::: {.callout-tip title="Exercise: Loop over a list"}
...
::: { .solution exercise="list-loop-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: list-loop-ex
#| check: TRUE
gradethis::grade_this_code("We mentioned that `for` loops can also be used for matrices. Let's put that to a test in the next exercise.")
```
...
::: {.callout-tip title="Exercise: Loop over a matrix"}
...
::: { .solution exercise="matrix-loop-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: matrix-loop-ex
#| check: TRUE
gradethis::grade_this_code("You're sufficiently comfortable with basic `for` looping, so it's time to step it up a notch!")
```
...
::: {.callout-tip title="Exercise: Mix it up with control flow"}
...
#| exercise: flow-loop
#| check: TRUE
gradethis::grade_this_code("In the next exercise, you'll customize this `for` loop even further with `break` and `next` statements.")
```
...
::: {.callout-tip title="Exercise: Next, you break it"}
...
::: { .solution exercise="break-for-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: break-for-ex
#| check: TRUE
gradethis::grade_this_code("`for`, `break`, `next`? We name it, you can do it!")
```
...
::: {.callout-tip title="Exercise: Build a for loop from scratch"}
...
::: { .solution exercise="build-loop-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: build-loop-ex
#| check: TRUE
gradethis::grade_this_code("This exercise concludes the chapter on `while` and `for` loops.")
```
...
check_question("2", 
               ...
               type = "radio", right = "Correct", wrong = "Incorrect")
```
...
```{r}
#| echo: false
check_question("It will return the vector `c(8, 32)`", 
               options = c("It will return 40.", 
...
::: {.callout-tip title="Exercise 1: Consult the documentation on the mean() function"}
...
::: {.callout-tip title="Exercise 2: Inspect the arguments of the mean() function"}
...
#| exercise: mean-args
#| check: TRUE
gradethis::grade_this_code("That wasn't too hard, was it? Take a look at the documentation and head over to the next exercise.")
```
...
::: {.callout-tip title="Exercise 1: Calculate average number of views"}
...
::: { .solution exercise="arg-pos-vs-name-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: Print average number of views"}
*Print out both* `avg_li` *and* `avg_fb`*.*

```{webr}
#| exercise: arg-pos-vs-name-ex-2
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
facebook <- c(17, 7, 5, 16, 8, 13, 14)
avg_li <- mean(linkedin)
...
::: { .solution exercise="arg-pos-vs-name-ex-2"}
::: { .callout-note collapse="false"}
...
#| exercise: arg-pos-vs-name-ex-2
#| check: TRUE
gradethis::grade_this_code("I'm sure you've already called more advanced R functions in your history as a programmer. Now you also know what actually happens under the hood.")
```
...
::: { .solution exercise="arg-mean-ex-1"}
::: { .callout-note collapse="false"}
...
::: { .solution exercise="arg-mean-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 3: Inspect both new variables"}
*Print out both* `avg_sum` *and* `avg_sum_trimmed`*; can you spot the difference?*

```{webr}
#| exercise: arg-mean-ex-3
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
facebook <- c(17, 7, 5, 16, 8, 13, 14)
avg_sum <- mean(linkedin + facebook)
...
::: { .solution exercise="arg-mean-ex-3"}
::: { .callout-note collapse="false"}
...
#| exercise: arg-mean-ex-3
#| check: TRUE
gradethis::grade_this_code("When the `trim` argument is not zero, it chops off a fraction (equal to `trim`) of the vector you pass as argument `x`.")
```
...
::: {.callout-tip title="Exercise 1: Basic average of linkedin"}
...
::: { .solution exercise="missing-values-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: Advanced average of linkedin"}
...
::: { .solution exercise="missing-values-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 1: Calculate the mean absolute deviation"}
...
::: { .solution exercise="compose-funcs"}
::: { .callout-note collapse="false"}
...
```{webr}
#| exercise: required-optional-exercise
#| caption: "R Code"
#| autorun: false

```

::: { .hint exercise="required-optional-exercise"}
::: { .callout-note collapse="false"}
```r
args(read.table)
```
...
#| echo: false
check_question(
  c("`header`, `sep` and `quote` are all optional arguments.",
    '`read.table("myfile.txt", "-", TRUE)` will throw an error.'), 
  options = c("`header`, `sep` and `quote` are all optional arguments.", 
...
::: {.callout-tip title="Exercise 1: Create a function pow_two()"}
*Create a function* `pow_two()`*: it takes one argument* `x` *and returns that number squared (that number times itself).*


```{webr}
#| exercise: create-func-ex-1
#| autorun: FALSE

```

::: { .solution exercise="create-func-ex-1"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 2: Use the function"}
*Call this newly defined function with* `12` *as input.*


```{webr}
#| exercise: create-func-ex-2
#| autorun: FALSE

```

::: { .solution exercise="create-func-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 3: Create a function sum_abs()"}
...
```{webr}
#| exercise: create-func-ex-3
#| autorun: FALSE

```

::: { .solution exercise="create-func-ex-3"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 4: Use the function"}
*Finally, call the function* `sum_abs()` *with arguments* `-2` *and* `3` *afterwards.*

```{webr}
#| exercise: create-func-ex-4
#| setup: TRUE
sum_abs <- function(a, b) {
  abs(a) + abs(b)  
}
...
::: { .solution exercise="create-func-ex-4"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise 1: Define the function hello()"}
*Define a function,* `hello()`*. It prints out "Hi there!" and returns* `TRUE`*. It has no arguments.*


```{webr}
#| exercise: define-hello-ex-1
#| autorun: FALSE

```

::: { .solution exercise="define-hello-ex-1"}
::: { .callout-note collapse="false"}
...
#| exercise: define-hello-ex-1
#| check: TRUE
gradethis::grade_this_code("Remember, the return() function exits the function with its argument as the output. The last line could just be `TRUE`, and `TRUE` would be returned as output by default - it is usually best practice to be explicit however and use return().")
```
...
::: {.callout-tip title="Exercise 2: Call the function hello()"}
*Call the function* `hello()`*, without specifying arguments of course.*

```{webr}
#| exercise: define-hello-ex-2
#| setup: TRUE
hello <- function() {
  print("Hi there!")
  return(TRUE)
...
::: { .solution exercise="define-hello-ex-2"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise: Finish the `pow_two()` function"}
...
::: { .solution exercise="finish-pow-two"}
::: { .callout-note collapse="false"}
...
#| exercise: finish-pow-two
#| check: TRUE
gradethis::grade_this_code("Have you tried calling this `pow_two()` function? Try `pow_two(5)`, `pow_two(5, TRUE)` and `pow_two(5, FALSE)`. Which ones give different results?")
```
...
check_question(
  ...
  type = "checkbox", right = "Correct", wrong = "Incorrect")
```
...
::: {.callout-tip title="Exercise: Define the interpret function"}
...
::: { .solution exercise="interpret-func-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: interpret-func-ex
#| check: TRUE
gradethis::grade_this_code("The annoying thing here is that `interpret()` only takes one argument. Proceed to the next exercise to implement something more useful.")
```
...
::: {.callout-tip title="Exercise 1: Define the `interpret_all()` function"}
...
```{webr}
#| exercise: interpret-all-func-ex
#| setup: TRUE
linkedin <- c(16, 9, 13, 5, 2, 17, 14)
facebook <- c(17, 7, 5, 16, 8, 13, 14)
```
...
::: { .solution exercise="interpret-all-func-ex"}
::: { .callout-note collapse="false"}
...
#| exercise: interpret-all-func-ex
#| check: TRUE
gradethis::grade_this_code("Have a look at the results; it appears that the sum of `views` on popular days are the same for Facebook and LinkedIn, what a coincidence! Your different social profiles must be fairly balanced.")
```
...
::: {.callout-tip title="Exercise: Peeking into function code"}
...
::: { .solution exercise="peek-into-funcs"}
::: { .callout-note collapse="false"}
...
::: {.callout-tip title="Exercise: A round pi"}
...
::: { .solution exercise="indiana-pi-ex"}
::: { .callout-note collapse="false"}
...
summarize(group_by(filter(select(read_csv("https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap03q14VasopressinVoles.csv"), c(treatment, percent)), percent >= 50), treatment), mean(percent))
```
...
summarize(
  group_by(
    filter(
      select(
        read_csv("https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap03q14VasopressinVoles.csv"), 
        c(treatment, percent)), 
      percent >= 50), 
    treatment), 
  mean(percent)
...
read.csv("https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap02q32ToxoplasmaAccidents.csv") |> table()
```

::: {.callout-tip title="Exercise 1: I love mosaic plots!"}

 * *First, use `read.csv` to load in the data located at https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap02q32ToxoplasmaAccidents.csv and store in the variable `toxoData`.*
...
#| exercise: toxo-ex-1
#| solution: TRUE
toxoData <- read.csv("https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap02q32ToxoplasmaAccidents.csv")
...
::: {.callout-tip title="Exercise 2: I love mosaic plots with pipes!"}
...
#| exercise: toxo-ex-2
#| solution: TRUE
read.csv("https://qubeshub.org/community/groups/introbiostats/File:/uploads/chap02q32ToxoplasmaAccidents.csv") %>%
  table() %>%
  mosaicplot()
```
...
#| exercise: toxo-ex-2
#| check: TRUE
gradethis::grade_this_code("This example illustrates another possible benefit of the pipe operator - avoiding intermediate variables. Note that sometimes the intermediate variables are useful, such as the contingency table `toxoTable` in this case.")
```
